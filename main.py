...
def load_environment():
    ...
    if not token:
        raise ValueError("未找到token环境变量，请在.env文件中配置")
    ...
    if admin_ids_str:
        try:
            ADMIN_IDS = [int(id_str.strip())
                         for id_str in admin_ids_str.split(",") if id_str.strip()]
        except ValueError:
            raise ValueError("ADMIN_IDS格式错误，应为逗号分隔的数字ID列表")

    return token, ADMIN_IDS


try:
    token, ADMIN_IDS = load_environment()
